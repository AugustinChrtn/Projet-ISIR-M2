...
world=np.load('Mondes/World_5.npy')
transitions=np.load('Mondes/Transitions_5.npy',allow_pickle=True)
...
for i in range(1):
        print(i)
        QA=Q_Agent(environment,alpha=0.2,beta=1,gamma=0.95,exploration='softmax')
        KAS=Kalman_agent_sum(environment,gamma=0.98,variance_ob=1,variance_tr=50,curiosity_factor=1)
        RA=Rmax_Agent(environment,gamma=0.9, max_visits_per_state=8, epsilon = 0.1,Rmax=1000)
        BEB=BEB_Agent(environment,gamma=0.9,beta=1000)
