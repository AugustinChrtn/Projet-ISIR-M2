...
class PEI_DE_Agent:
    ...
    def __init__(self,environment, gamma=0.95,gamma_de=0.1,coeff_de=1,epsilon=0.1,step_update=10):
        
        self.environment=environment
        self.gamma = gamma
        self.gamma_de=gamma_de
        ...
        self.DE=defaultdict(lambda: defaultdict(lambda: 0.0))
        self.epis=defaultdict(lambda: defaultdict(lambda: 0.0))
        self.coeff_de=coeff_de
    ...
    def learn(self,old_state,reward,new_state,action):
                    ...
                    if self.nSA[old_state][action]>self.step_update:
                        ...
                        self.DE[old_state][action]=max(self.entropy(old_prior_dict)-self.entropy(self.prior[old_state][action]),1e-3)
                    ...
                    while delta > 1e-3 :
                        ...
                        for visited_state in self.nSA:
                            for taken_action in self.nSA[visited_state]:
                                ...
                                self.epis[visited_state][taken_action]=self.DE[visited_state][taken_action]+self.gamma_de*np.sum([max(self.Q[next_state].values())*self.tSAS[visited_state][taken_action][next_state] for next_state in self.tSAS[visited_state][taken_action]])
    ...
    def choose_action(self):
        ...
        if np.random.random() > (1-self.epsilon) :
            ...
            total_dict={k:q_values[k]+self.coeff_de*epis_values[k] for k in q_values}
    ...
    def ajout_states(self):
        ...
        for state_1 in self.states:
            for action in self.environment.actions:
                ...
                self.DE[state_1][action]=self.entropy(self.prior[state_1][action])
                self.epis[state_1][action]=self.DE[state_1][action]/(1-self.gamma_de)

    def entropy(self,prior):
        sum_prior=sum(prior.values())
        values=np.array(list(prior.values()))/sum_prior
